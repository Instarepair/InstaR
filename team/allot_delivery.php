...
$merchant=$arr4[0]['pickupperson'];
            $cmd="select * from techyuser where personcode='$vendorcode'";
...
$cmd="Update transaction set deliveryperson='$vendorcode', where trans_id='$orderid'";
$result=mysql_query($cmd);
$show_notify=1;
$optitle='DELIVERY ALLOTED';
$opbody='Congratulations admin ! You have successfully alloted '.$pickupboyname.' for the delivery of order id '.$orderid;
...
$link='www.instarepair.in/panels.php';
  $customermessage="Dear ".$customer." your gadget is scheduled to delivery between 1 to 2 hours. Transaction ID : ".$orderid.",Techie name :".$pickupboyname."mobile no :".$phone_merchant;
sendmessage($customermessage,$phone_customer);
// echo $customermessage;

  $merchantmessage="Transaction ID : ".$orderid.", Customer Name : ".$customer." Phone : ".$phone_customer.", Order details : ".$devicetype.", . deliver address- ".$address.".Press Link When Picked ".$link;
...
$bill=get_bill_details($orderid);
       ...
       $cmdval="SELECT `GCMKey` FROM `TechyUser` WHERE UserId='$vendorcode'";
      ...
      $cmd1="INSERT INTO `techypickupdelivery`(`UserId`, `Type`, `SRN`, `TerminalAddress`, `CustomerAddress`, `Name`, `Mobile`, `Modelname`,`Amount`) 
      VALUES ('$vendorcode','$pickup','$orderid','T3','$address','$address2','$address1','$brandname','$bill')";
      $result1=mysql_query($cmd1);

    $cmd="update transaction set trackstatus='Dispatched' where trans_id='$orderid'";
...
$trans_id=$_GET['orderid'];
        ...
        $order=get_order_details($_GET['orderid']);
...
<!-- ================== add a new person ================================================================================ -->
    ...
    display: <?php if(isset($show_notify))
echo 'none';
else
echo 'block';    ?>;
...
<div class="container" style='margin-top:-12vw'>
  ...
  <table class="table" style='font-size: 11px;'>
    ...
    <tbody>
      ...
      ?>">
        <td><?php echo $r+1;?></td>
        <td><?php echo $arr1[$r]['Name'].'( '.$arr1[$r]['UserId'].' )';?></td>
