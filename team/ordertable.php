...
if(isset($_GET['pickedupnotify'])){
$orderid=$_GET['pickedupnotify'];

$msgcustomer="SRN: ".$orderid." Your gadget has reached Repair Center and will be Diagnosed shortly. You will be soon contacted by our executive with the estimate.";
$phonecustomer=get_phone_customer_from_transaction($orderid);
sendmessage($msgcustomer,$phonecustomer);
...
$redirectlink=$_POST['redirectlink'];
      ...
      $orderid=mysql_real_escape_string($_POST['trackid']);
        $t1=mysql_real_escape_string($_POST['customername']);
        $t2=mysql_real_escape_string($_POST['bill']);
        $t3=mysql_real_escape_string($_POST['link']);
...
// check whether the  redirect link is set or not =====\\\
...
window.location='<?php echo $redirectlink; ?>';
 ...
 if(isset($_GET['cancel']))
                 ...
                 $msgcustomer="SRN: ".$cancel." Your Cancellation Request is under process";
$phonecustomer=get_phone_customer_from_transaction($cancel);
sendmessage($msgcustomer,$phonecustomer);             
...
<body>
  ...
  <!-- =- == div for laoding the netire oreder detauil over starthere ============================= -->


           <div id='order_items_show' >
          ...
          <!-- === div for loading entire order  endshere ======================================= -->
...
<center>
   <select name='trackstatus' id='currentstatusid' style='margin-bottom:12px;' class='input2' onchange="get_status_and_load_form()">
...
<a href="?sendpickupnotify=1" id='pickeduplink' style='background: black;
    color: white;
    text-transform: uppercase;
    padding: 3px;
    margin-top: 1'>send pick up notify</a>
           </div>

<!-- ========== dispatched form starts here ======================================================================= -->
...
if($orderarr[$r]['status']!='cancelled'){
echo $orderarr[$r]['trans_id'].' '."<span style='color:red'>".$orderarr[$r]['trackstatus']."</span>".' '."<span style='color:green'>".$orderarr[$r]['bookingdate']."</span>";
}
else
...
?>
<span style='color:red'>( <?php echo $orderarr[$r]['phonenumber'];?> )</span>
</td> 
<td><?php 
echo $orderarr[$r]['remark'];
...
?><a href='editorder.php?trans_id=<?php echo $orderarr[$r]['trans_id'];?>' class='edit_small'>EDIT</a>

<a href='javascript:void(0)' onclick="load_items_for_orderid('<?php echo $orderarr[$r]['trans_id'];?>')" class='edit_small'>VIEW</a>
<a href="?cancel=<?php echo $orderarr[$r]['trans_id'];?>"  class='edit_small'>CANCEL</a><BR>
</td>
  ...
  document.getElementById('trackorderstatuschange').style.display='block';
    ...
    document.getElementById('trackidfordispatch').value='<?php echo $orderarr[$r]['trans_id'];?>';
...
<!-- ========== forward to pickup or the vendo ka loop starts here =========================== -->



  <?php 
                          if($orderarr[$r]['trackstatus']=='in Process')
                          { ?>

      <td><a href="admin_panelnew.php?forward_id=<?php echo $orderarr[$r]['trans_id'];?>" target='_blank' onclick="
      " class="btn btn-default" 
      style="background-color:green;color:white;opacity:0.78">
                      <?php 
                          if($orderarr[$r]['trackstatus']=='in Process')
                          {
                                if($orderarr[$r]['pickupperson']==NULL)
...
<!-- ========================== in case of assigned process change ========================== -->
<?php 
  if($orderarr[$r]['trackstatus']=='Assigned')
                          { ?>

     <td><a href="allot_vendornew.php?forward_id=<?php echo $orderarr[$r]['trans_id'];?>" target='_blank' onclick="
      " class="btn btn-default" 
      style="background-color:orange;color:white;opacity:0.78">
                        ...
                        if($orderarr[$r]['trackstatus']=='Assigned')
                                {
                                   if($orderarr[$r]['vendor_alloted']!=NULL)
                                       echo 'VENDOR ALLOTED';
                                     else
...
<!-- =======  forward to pickup or the vendor ka loop ends here =============================== -->
...
document.getElementById('secondform').style.display='none';
    var link= document.getElementById("pickeduplink");
   link.setAttribute("href", "ordertable.php?pickedupnotify="+r);
}
...
function load_items_for_orderid(orderid)
{
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
    ...
    document.getElementById("order_items_show").innerHTML=xmlhttp.responseText;

    }

  }
xmlhttp.open("GET","show_order.php?orderid="+orderid,true);
xmlhttp.send();


}

function create_link_for_deliveryid(){

       var r=document.getElementById('transidchange').value;
window.location='allot_delivery.php?orderid='+r;
